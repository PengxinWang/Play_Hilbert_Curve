...
def right_shift(binary, k=1, axis=-1):
    ...
    e.g.:
    ------
    binary = torch.tensor([[1,0,1,0], [0,1,0,1]]), k=2, axis=-1
    shifted_binary = torch.tensor([[0,0,1,0],[0,0,0,1]]) 
     """
...
def encode(locs, num_dims, num_bits):
    ...
    -------
     locs - An ndarray of locations in a hypercube of num_dims dimensions, in
            ...
            num_dims.
            e.g. torch.tensor([[0,0], [0,1], [2,2]])

     num_dims - The dimensionality of the hypercube. Integer.

     num_bits - The number of bits for each dimension. Integer.
            e.g. a 32*32 image, then num_bits >= 5

    Returns:
    --------
     The output is an ndarray of uint64 integers with the same length as the
     input. e.g. torch.tensor([0, 1, 5])
    """
...
def decode(hilberts, num_dims, num_bits):
    ...
    if num_dims * num_bits > 64:
        raise ValueError(
            f"""
      num_dims={num_dims} and num_bits={num_bits} for {num_bits*num_dims} bits total, which can't be encoded
      into a uint64.  Are you sure you need that many points on your Hilbert curve?
      """
      )
    ...
    for bit in range(num_bits - 1, -1, -1):
        ...
        for dim in range(num_dims - 1, -1, -1):
            ...
            # Where the bit is off, exchange the lower bits with the 0 dimension.
            to_flip = torch.logical_and(
                torch.logical_not(mask[:, None].expand(-1, gray.shape[-1]-bit-1)), # align the shape
                torch.logical_xor(gray[:, 0, bit + 1 :], gray[:, dim, bit + 1 :]),
            )
